...
function RecruiterLogin() {
  ...
  const { login, isLoading: apiLoading, error: apiError, clearError } = useAuthApi();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        // Accept both accessToken and access_token for compatibility
        ...
        if (newAttempts >= MAX_ATTEMPTS) {
            ...
            setErrors({ 
                general: apiError || `Invalid credentials. ${MAX_ATTEMPTS - newAttempts} attempts remaining.` 
            });
        }
    }
};
  ...
  useEffect(() => {
    ...
    if (session) {
      const { email, rememberMe } = JSON.parse(session);
      if (rememberMe) {
